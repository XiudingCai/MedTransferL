...
def main(args: argparse.Namespace):
    ...
    lr_scheduler_d = LambdaLR(optimizer_d,
                              lambda x: (1. - float(x) / args.epochs / args.iters_per_epoch) ** (args.lr_power))
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        dann.eval()
        y_s = model(x_s)
        # print(x_s.shape, y_s.shape)  # (8, 3, 256, 256) (8, 5, 33, 33)
...
if __name__ == '__main__':
    ...
    # dataset parameters
    parser.add_argument('--dataset_root', help='root path of the source dataset')
    ...
    parser.add_argument('--train-size', nargs='+', type=int, default=(256, 256),
                        help='the input and output image size during training')
    parser.add_argument('--test-input-size', nargs='+', type=int, default=(256, 256),
                        help='the input image size during test')
    parser.add_argument('--test-output-size', nargs='+', type=int, default=(256, 256),
